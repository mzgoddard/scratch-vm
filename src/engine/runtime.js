...
class Runtime extends EventEmitter {
    ...
    _pushThread (id, target, opts = {}) {
        ...
        thread.stackClick = opts.stackClick || false;
        thread.updateMonitor = opts.updateMonitor || false;
    ...
    _restartThread (thread) {
        ...
        thread.target.replaceThread(thread, newThread);
    ...
    startHats (requestedHatOpcode,
        ...
        this.allScriptsByOpcodeDo(requestedHatOpcode, (script, target) => {
            ...
            if (hatMeta.restartExistingThreads) {
                ...
                for (let i = 0; target.threads.length; i++) {
                    if (target.threads[i].topBlock === topBlockId &&
                        // stack click threads and hat threads can coexist
                        !target.threads[i].stackClick) {
                        newThreads.push(target._restartThread(target.threads[i]));
                        return;
                    }
                }
            } else {
                ...
                for (let j = 0; j < target.threads.length; j++) {
                    if (target.threads[j].topBlock === topBlockId &&
                        // stack click threads and hat threads can coexist
                        !target.threads[j].stackClick &&
                        target.threads[j].status !== Thread.STATUS_DONE) {
